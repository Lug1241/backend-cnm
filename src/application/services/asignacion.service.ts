...
export class AsignacionService {
    ...
    async update(id: number, dto: UpdateAsignacionDto): Promise<Asignacion> {
        ...
        const asignacionesDocente = await this.asignacionRepository.findByDocente(docente);
        const asignacionesDelPeriodo = asignacionesDocente.data.filter(
            asig => asig.periodoAcademico?.id === dto.ID_periodo_academico && asig.id !== id
        );
    ...
    async getByMateria(id_periodo: number, nivel: any, materia: string, jornada: any) {
        const periodoDummy = { id: id_periodo } as any;
        return this.asignacionRepository.findByMateria(periodoDummy, nivel, materia, jornada);
    }
