...
export class FechaProcesoRepository implements IFechaProcesoRepository {
    ...
    async update(id: number, fechaProceso: Partial<FechaProceso>): Promise<FechaProceso> {
    ...
    async findLatestByProceso(proceso: TipoProceso): Promise<FechaProceso | null> {
    ...
    private toDomain(ormEntity: FechaProcesoOrmEntity | null): FechaProceso | null {
        if (!ormEntity) return null;
